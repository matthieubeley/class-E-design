...
%BONUS : Analytical derivation of the parameters for the input current RMS value
...
%BONUS : Analytical derivation of the parameters for the capacitor current RMS value
C1 = k1.^2.*(pi*(1-D)+1/4./q.*(sin(4*pi*q.*D)-sin(4*pi*q)));
C2 = k2.^2.*(pi*(1-D)+1/4./q.*(sin(4*pi*q)-sin(4*pi*q.*D)));
C3 = (q.*p./(q.^2-1)).^2.*(pi*(1-D)+1/4*(sin(4*pi*D+2*phi)-sin(2*phi)));
C4 = k1.*k2/2./q.*(cos(4*pi*q)-cos(4*pi*q.*D));
C5 = 2*k1.*q.*p./(q.^2-1).^2.*(-sin(2*pi*q.*D).*cos(2*pi*D+phi)+sin(2*pi*q).*cos(phi)-q.*cos(2*pi*q).*sin(phi)+q.*cos(2*pi*q.*D).*sin(2*pi*D+phi));
C6 = -2*k2.*q.*p./(q.^2-1).^2.*(-cos(2*pi*q.*D).*cos(2*pi*D+phi)+cos(2*pi*q).*cos(phi)+q.*sin(2*pi*q).*sin(phi)-q.*sin(2*pi*q.*D).*sin(2*pi*D+phi));
...
Ic_RMS_norm = sqrt(2/pi)*g./q./p.*sqrt(C1+C2+C3+C4+C5+C6);
